...
from utils.report_html import save_html_report
...
async def run_scans(args):
    ...
    if run_all or args.subdomains: tasks["subdomains"] = scan_subdomains(args.target, args.timeout, args.threads)
    if run_all or args.headers:    tasks["headers"]    = scan_headers(args.target, args.timeout)
    if run_all or args.ports:      tasks["ports"]      = scan_ports(args.target, args.timeout, args.threads)
    if run_all or args.dns:        tasks["dns"]        = scan_dns(args.target)
    if run_all or args.whois:      tasks["whois"]      = scan_whois(args.target)
    if run_all or args.tech:       tasks["tech"]       = scan_tech(args.target, args.timeout)
    if run_all or args.waf:        tasks["waf"]        = scan_waf(args.target, args.timeout)
    if run_all or args.email:      tasks["email"]      = scan_email_security(args.target)
    if run_all or args.ssl:        tasks["ssl"]        = scan_ssl(args.target, args.timeout)
    if run_all or args.cors:       tasks["cors"]       = scan_cors(args.target, args.timeout)
    if run_all or args.robots:     tasks["robots"]     = scan_robots(args.target, args.timeout)
...
def print_summary(risk: dict):
    """Print a clean risk summary panel at the end of every scan."""
    ...
    lines = [
        f"[bold]Risk Grade:[/bold] [{color}]{grade}[/{color}]  [bold]Score:[/bold] [{color}]{score}/100[/{color}]",
        f"[bold]Total Findings:[/bold] {total}",
        ""
    ]
    for sev, count in summary.items():
        if count:
            c = {"CRITICAL": "red", "HIGH": "red", "MEDIUM": "yellow", "LOW": "green", "INFO": "blue"}.get(sev, "white")
            lines.append(f"  [{c}]{sev}[/{c}]: {count}")

    console.print(Panel("\n".join(lines), title="📊 Risk Summary", border_style=color))
...
def print_results(results):
    ...
    if "subdomains" in results:
        ...
        t.add_column("Subdomain", style="cyan"); t.add_column("IP", style="white")
        if isinstance(data, list):
            for item in data:
                if isinstance(item, dict): t.add_row(item.get("subdomain",""), item.get("ip",""))
                else: t.add_row(str(item), "")
        console.print(t)
    ...
    if "ports" in results:
        ...
        t.add_column("Port",style="cyan"); t.add_column("Service",style="white"); t.add_column("State",style="green")
        if isinstance(data, list):
            for item in data:
                if isinstance(item, dict): t.add_row(str(item.get("port","")), item.get("service",""), item.get("state","open"))
        console.print(t)

    if "dns" in results:
        data = results["dns"]
        t = Table(title="DNS Records", box=box.SIMPLE_HEAD)
        t.add_column("Type",style="cyan"); t.add_column("Value",style="white",overflow="fold")
        if isinstance(data, dict):
            for rtype, records in data.items():
                for r in records: t.add_row(rtype, str(r))
        console.print(t)

    if "whois" in results:
        data = results["whois"]
        t = Table(title="WHOIS", box=box.SIMPLE_HEAD)
        t.add_column("Field",style="cyan"); t.add_column("Value",style="white",overflow="fold")
        if isinstance(data, dict):
    ...
    if "tech" in results:
        ...
        t.add_column("Category",style="cyan"); t.add_column("Detected",style="white")
        if isinstance(data, dict):
            for k, v in data.items(): t.add_row(k, ", ".join(v) if isinstance(v, list) else str(v))
        console.print(t)
    ...
    if "email" in results:
        ...
        t.add_column("Check",style="cyan"); t.add_column("Status",style="bold"); t.add_column("Value",style="white",overflow="fold")
        if isinstance(data, dict):
            for k, v in data.items():
                if isinstance(v, dict):
                    status = v.get("status","")
    ...
    if "ssl" in results:
        ...
        if isinstance(data, dict) and "error" not in data:
            ...
            if risks:
                ...
                rt.add_column("Issue",style="white",overflow="fold"); rt.add_column("Severity",style="bold")
                for r in risks:
                    sev = r.get("severity","")
    ...
    if "cve" in results:
        ...
        if isinstance(data, dict) and data.get("findings"):
            ...
            console.print(Panel(f"[{color}]{overall}[/{color}]  —  {data.get('total_cves',0)} CVE(s)  [dim](source: {data.get('source','')})[/dim]", title="CVE Lookup"))
            for finding in data["findings"]:
                ft = Table(title=f"CVEs — {finding['tech']}", box=box.SIMPLE_HEAD)
                ft.add_column("CVE ID",style="cyan"); ft.add_column("Severity",style="bold"); ft.add_column("Description",style="white",overflow="fold")
    ...
    if "robots" in results:
        ...
        if isinstance(data, dict):
            ...
            info = (f"robots.txt: {'[green]found[/green]' if robots.get('found') else '[dim]not found[/dim]'}  "
            ...
            if interesting:
                ...
                for path in interesting[:30]: it.add_row(path)
                console.print(it)
            ...
            if disallowed:
                ...
                for path in disallowed[:20]: dt.add_row(path)
                console.print(dt)
...
async def main():
    ...
    if args.delete_profile:
        delete_profile(args.delete_profile); return
    if not args.target:
        console.print("[red]Error:[/red] Please provide a target domain.")
        console.print("Usage: [bold]python main.py <target> [options][/bold]"); return
    if args.save_profile:
        save_profile(args.save_profile, args, description=args.desc); return

    if args.profile:
        profile = get_profile(args.profile)
        if not profile:
            console.print(f"[red]Unknown profile '[cyan]{args.profile}[/cyan]'.[/red]  "
                          f"Run [bold]python main.py --list-profiles[/bold] to see available profiles."); return
    ...
    console.print(f"[bold]Target:[/bold] [cyan]{args.target}[/cyan]  "
                  f"[bold]Timeout:[/bold] {args.timeout}s  [bold]Threads:[/bold] {args.threads}\n")
    ...
    # ── Reports ───────────────────────────────────────────────────────────────
    if args.output == "html":
        path = save_html_report(results, risk)
        console.print(f"\n[bold green]HTML report saved:[/bold green] {path}")
    elif args.output == "json":
        path = generate_report(results, fmt="json")
